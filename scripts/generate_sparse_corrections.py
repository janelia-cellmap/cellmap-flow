...
4. Saves all corrections in a single zarr with timestamp
...
def sample_points_from_mask(
    mask: np.ndarray, num_points: int, min_distance: int = 5
) -> np.ndarray:
    """
...
def create_background_mask(
    mito_mask: np.ndarray, min_distance: int = 2, max_distance: int = 10
) -> np.ndarray:
    """
...
def create_sparse_annotation_mask(
    ...
    sphere = (
        (z_grid - center) ** 2 + (y_grid - center) ** 2 + (x_grid - center) ** 2
    ) <= annotation_radius**2
...
def add_ome_ngff_metadata(group, name, voxel_size, translation_offset=None):
    ...
    # Add scale first
    transforms.append({"type": "scale", "scale": voxel_size.tolist()})

    # Then add translation if provided
    if translation_offset is not None:
        physical_translation = (
            np.array(translation_offset) * np.array(voxel_size)
        ).tolist()
        transforms.append({"type": "translation", "translation": physical_translation})
...
def main():
    ...
    ANNOTATION_RADIUS = 1  # Radius of annotation sphere around each point
    ...
    output_path = OUTPUT_DIR / f"sparse_corrections_{timestamp}.zarr"
    ...
    output_root.attrs.update(
        {
            ...
            "description": "Sparse point corrections for partial annotation training",
        }
    )
    ...
    for idx, source_corr_id in enumerate(correction_ids):
        print(
            f"[{idx+1}/{len(correction_ids)}] Processing correction {source_corr_id}..."
        )

        # Load data
        corr_group = input_root[source_corr_id]
        ...
        foreground_points = sample_points_from_mask(
            mito_binary, NUM_FOREGROUND_POINTS, min_distance=MIN_POINT_DISTANCE
        )
        ...
        background_voxels_annot = np.sum(sparse_mask == 1)
        ...
        print(f"    - Background (1): {background_voxels_annot:,}")

        # Generate unique correction ID
        new_corr_id = str(uuid.uuid4())

        # Create group in output zarr
        ...
        add_ome_ngff_metadata(
            mask_group, "mask", voxel_size, translation_offset=offset_diff.tolist()
        )
        ...
        add_ome_ngff_metadata(
            ...
            "prediction",
            voxel_size,
            translation_offset=offset_diff.tolist(),
        )
        ...
        corr_output.attrs.update(
            {
                ...
                "source_correction": source_corr_id,
        ...
        print(f"  ✓ Added as correction {corrections_created}")
        print()

    # Update global metadata
    output_root.attrs["num_corrections"] = corrections_created

    print("=" * 60)
    print(f"✓ Complete! Created {corrections_created} sparse corrections")
    print(f"Output: {output_path}")
    print("=" * 60)
