...
raw = ImageDataInterface(
    "/nrs/cellmap/data/jrc_mus-liver-zon-1/jrc_mus-liver-zon-1.zarr/recon-1/em/fibsem-uint8/s2"
).ts
...
class InteractiveInference:
    def __init__(self):
        ...
        self.inf_results = zarr.zeros(
            self.raw.roi.shape, chunks=(64, 64, 64), dtype=np.uint8
        )
...
""",
    ...
    def _do_inference(self, action_state):
        ...
        raw = self.raw[slice_expr][..., 0]
        boundary_mask = raw == 0
        boundary_mask[:, :, :-1] |= raw[:, :, :-1] != raw[:, :, 1:]
        boundary_mask[:, :, 1:] |= raw[:, :, :-1] != raw[:, :, 1:]
        boundary_mask[:, :-1, :] |= raw[:, :-1, :] != raw[:, 1:, :]
        boundary_mask[:, 1:, :] |= raw[:, :-1, :] != raw[:, 1:, :]
        boundary_mask[:-1, :, :] |= raw[:-1, :, :] != raw[1:, :, :]
