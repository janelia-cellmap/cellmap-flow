...
class ModelConfig:
    ...
    def __str__(self) -> str:
        elms = ", ".join(f"{k}: {v}" for k, v in vars(self).items())
        return f"{type(self).__name__}({elms})"
    ...
    def _validate_config(self):
        ...
        required = ['read_shape', 'write_shape', 'input_voxel_size', 
                    'output_voxel_size', 'output_channels', 'block_shape']
    ...
    def output_dtype(self):
        ...
        logger.warning(f"Model {self.name} does not define output_dtype, defaulting to np.float32")
        return np.float32
...
class ScriptModelConfig(ModelConfig):
    ...
    def _get_config(self):
        ...
        if not hasattr(config, "read_shape"):
            config.read_shape = Coordinate(config.input_size) * Coordinate(config.input_voxel_size)
        if not hasattr(config, "write_shape"):
            config.write_shape = Coordinate(config.output_size) * Coordinate(config.output_voxel_size)
        if not hasattr(config, "block_shape"):
            config.block_shape = np.array(tuple(config.output_size) + (config.output_channels,))
        return config
...
class FlyModelConfig(ModelConfig):
    ...
    def _get_config(self):
        ...
        config.block_shape = np.array(tuple(self.output_size) + (config.output_channels,))
        return config
...
class BioModelConfig(ModelConfig):
    ...
    def _get_config(self):
        ...
        (config.input_name, config.input_axes, config.input_spatial_dims,
         config.input_slicer, is_2d_with_batch) = self.load_input_information(config.model)

        (config.output_names, config.output_axes, config.block_shape,
         config.output_spatial_dims, config.output_channels) = self.load_output_information(config.model)

        if self.voxels_to_process:
            if not is_2d_with_batch:
                warnings.warn("edge_length_to_process is only supported for 2D models")
            else:
                batch_size = max(1, self.voxels_to_process // np.prod(config.input_spatial_dims))
        ...
        config.read_shape = Coordinate(config.input_spatial_dims) * config.input_voxel_size
        config.write_shape = Coordinate(config.output_spatial_dims) * config.output_voxel_size
    ...
    def load_input_information(self, model):
        ...
        input_name, input_axes, input_dims, is_2d_with_batch = self.get_axes_and_dims(input_sample)
        input_spatial_dims = self.get_spatial_dims(input_axes, input_dims)
        input_slicer = self.get_input_slicer(input_axes)
        return input_name, input_axes, input_spatial_dims, input_slicer, is_2d_with_batch
    ...
    def load_output_information(self, model):
        ...
        output_spatial_dims = [output_dims[finalized_output_axes.index(a)] for a in ["z", "y", "x"]]
        output_channels = output_dims[finalized_output_axes.index("c")]
        block_shape = [output_dims[finalized_output_axes.index(a)] for a in ["z", "y", "x", "c"]]
        return output_names, output_axes, block_shape, output_spatial_dims, output_channels
    ...
    def get_axes_and_dims(self, sample):
        ...
        for sample_axis_to_dim_dict in sample_axis_to_dims_dicts:
            ...
            if ("b" in current_sample_axes or "batch" in current_sample_axes) and "z" not in current_sample_axes:
    ...
    def get_input_slicer(self, input_axes):
        return tuple(
            np.newaxis if a.startswith("c") or (a == "b" and "z" in input_axes) else slice(None)
            for a in input_axes
        )
...
def concat_along_c(arrs, axes_list, channel_axis_name="c"):
    ...
    c_index = next((axes.index(channel_axis_name) for axes in axes_list if channel_axis_name in axes), 0)
...
def reorder_axes(
    ...
    for i in reversed(range(len(axes))):
        if axes[i] not in desired_order:
            if arr.shape[i] != 1:
                raise ValueError(f"Cannot remove axis '{axes[i]}' with size {arr.shape[i]} (must be 1).")
...
def process_chunk_bioimage(self, idi: ImageDataInterface, input_roi: Roi):
    ...
    output = predict(model=self.model, inputs=input_sample, skip_preprocessing=bool(input_sample.stat))
...
def format_output_bioimage(self, output_sample, output_names=None, output_axes=None):
    ...
    output, reordered_axes = reorder_axes(output, output_axes, desired_order=["c", "z", "y", "x"])
