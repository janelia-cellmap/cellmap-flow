"""A stand-alone U-Net isolated from dacapo pipeline.
...
class UNetLSD(torch.nn.Module):
    """This CNNectomeUNet wrapper does the following:

        1. Adds an LSD prediction head to the UNet.
        2. Makes sure state_dict keys match dacapo pre-trained checkpoints.
        3. Optionally applies sigmoid to the UNet output at eval time.

    The CNNectomeUNet class below is a clean torch.nn.Module and it can be
    ...
    def __init__(self,
        ...
        # The particular names `architecture`, `prediction_head`, `chain` are
        # needed for state_dict consistency with pre-trained dacapo models.
...
class CNNectomeUNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self._eval_shape_increase = eval_shape_increase
...
model = UNetLSD()
status = load_checkpoint_from_path(model, 
                                   "/groups/espinosamedina/home/aurrecoecheas/jrc-muss-pancreas-innervation/train_fly_model/runs/run02/model_checkpoint_260000",
...
read_shape = gp.Coordinate( 196, 196, 196) * Coordinate(input_voxel_size)
write_shape = gp.Coordinate(92, 92, 92) * Coordinate(input_voxel_size)
output_voxel_size = Coordinate((16, 16, 16))
block_shape = np.array((92, 92, 92,output_channels))
model = model.to(device)
