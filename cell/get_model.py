...
#%%
config_path = "/nrs/funceworm/vaxenburgr/experiments/funceworm-lite-adult-db2/e0ece3c0e5d14aa2877215867f9c7d52/configs/network_config.json"
chckpoint_path= "/nrs/funceworm/vaxenburgr/experiments/funceworm-lite-adult-db2/e0ece3c0e5d14aa2877215867f9c7d52/checkpoints/48401"
"""An affinity-LSD U-Net model.

Example usage:

model = UNetLSD()
status = load_checkpoint_from_path(model, 'path/to/checkpoint')
print(status)  # Should print "<All keys matched successfully>"

x = torch.rand((1, 1, 196, 196, 196))
y = model(x)
print(y.shape)  # Should print "torch.Size([1, 13, 92, 92, 92])"
...
def load_checkpoint_from_path(model: torch.nn.Module,
                              ckpt_path: str,
                              weight_key: str = 'model',
                              map_location: str | None = None,
                             ) -> torch.nn.modules.module._IncompatibleKeys:
    """Load checkpoint to model directly from path.
    
    Args:
        model: A pytorch model.
        ckpt_path: Path to the checkpoint file.
        weight_key: Key in the checkpoint dict to load weights from.
        map_location: See pytorch docs,
            https://pytorch.org/docs/stable/generated/torch.load.html
    """
    ckpt = torch.load(ckpt_path, map_location=map_location)
    status = model.load_state_dict(ckpt[weight_key])
    return status


class UNetLSD(torch.nn.Module):
    """This class is a CNNectomeUNet wrapper and it does the following:

        1. Adds a prediction head to the UNet, for example for affinities + LSDs.
        2. Initializes weights with Kaiming normal (pytorch default is Kaiming uniform).
        3. Optionally applies sigmoid to the UNet output at eval time.

    The CNNectomeUNet class below is an independent torch.nn.Module and it can be
    also used directly as a stand-alone model, without this wrapper.
    """
    ...
    def __init__(self,
                 ...
                 prediction_head_out_channels=13,
                 use_attention=False,
                 batch_norm=False,
                 equivariant=False,
                 sigmoid_on_output_at_eval=False,
                 positional_embedding: bool = False,
                 ):
        """Wrap the CNNectomeUNet model, see CNNectomeUNet class below for detail."""
        
        super().__init__()

        self.architecture = CNNectomeUNet(
            fmaps_in=fmaps_in,
            num_fmaps=num_fmaps,
            fmaps_out=fmaps_out,
        ...
        del positional_embedding  # Not used.
        self.prediction_head = torch.nn.Conv3d(in_channels=fmaps_out,
                                               out_channels=prediction_head_out_channels,
                                               kernel_size=1)
        self.chain = torch.nn.Sequential(self.architecture, self.prediction_head)
        self._eval_activation = torch.nn.Sigmoid() if sigmoid_on_output_at_eval else None

        # Initialize weights with Kaiming normal (pytorch default is Kaiming uniform).
    ...
    def forward(self, x):
        """Forward pass."""
        x = self.chain(x)
        if not self.training and self._eval_activation is not None:
            x = self._eval_activation(x)
        return x


class CNNectomeUNet(torch.nn.Module):
    """
    ...
    def __init__(self,
        ...
        if downsample_factors is None:
            downsample_factors = downsample_factors=[(2, 2, 2), (2, 2, 2), (3, 3, 3)]
        if kernel_size_down is None:
            kernel_size_down = [[(3, 3, 3)] * 2] * 4
        if kernel_size_up is None:
            kernel_size_up = [[(3, 3, 3)] * 2] * 3
    ...
    def module(self):
        ...
        if len(self.upsample_factors) > 0:
            ...
            for upsample_factor in self.upsample_factors:
                up = Upsample(
                    tuple(upsample_factor),
                    mode="nearest" if self.constant_upsample else "transposed_conv",
                ...
                conv = ConvPass(
                    ...
                    activation=self.activation,
                    batch_norm=self.batch_norm,
                    residual_connection=self.residual_connections,
                )
                layers.append(conv)
...
class CNNectomeUNetModule(torch.nn.Module):
                    ...
                    [
                        Upsample(
                            ...
                            ** (level + (1 - upsample_channel_contraction[level])),
                            crop_factor=crop_factors[level] if self.equivariant else None,
                            next_conv_kernel_sizes=kernel_size_up[level] if self.equivariant else None,
    ...
    def rec_forward(self, level, f_in):
        """
        *** This is the original recursive forward pass. It's not called anymore. ***
    ...
    def forward(self, x):
        ...
        # *** This is the original recursive call. It's not used anymore. ***
        # *** Below is the same logic implemented without recursion. ***
        # y = self.rec_forward(self.num_levels - 1, x)
        # if self.num_heads == 1:
        #     return y[0]
        # return y

        # Forward pass without recursion.

        # Down.
        f_left = []
        for i in range(self.num_levels-1):
            x = self.l_conv[i](x)  # Conv block.
            f_left.append(x)
            x = self.l_down[i](x)  # Downsample.
        ...
        # Up.
        end_idx = self.num_levels - 2
        for i in range(self.num_levels-1):
            if self.use_attention:
                x_attented = [
                    self.attention[h][end_idx-i](x[h], f_left[end_idx-i])
                    for h in range(self.num_heads)]
                # Upsample.
                x = [self.r_up[h][end_idx-i](x[h], x_attented[h])
                     for h in range(self.num_heads)]
            else:
                # Upsample.
                x = [self.r_up[h][end_idx-i](x[h], f_left[end_idx-i])
                     for h in range(self.num_heads)]
            # Conv block.
            x = [self.r_conv[h][end_idx-i](x[h]) for h in range(self.num_heads)]
...
class ConvPass(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.residual_connection:
            if in_channels != out_channels:
                assert len(kernel_sizes[0]) == 3, (
                    "Residual projection only implemented for 3D convs for now.")
        ...
        for i, kernel_size in enumerate(kernel_sizes):
            ...
            if activation is not None and i < len(kernel_sizes) - 1:
                layers.append(activation())

        self.conv_pass = torch.nn.Sequential(*layers)
        self.last_activation = activation() if activation is not None else nn.Identity()

    def center_crop_3d(self, x, target_size):
        """
        Crop x to match target_size.
        ...
        return x[
            :, :,
            crop_h : crop_h + target_h,
            crop_w : crop_w + target_w,
            crop_d : crop_d + target_d,
        ]
    ...
    def forward(self, x):
        ...
        if not self.residual_connection:
            ...
            x = self.conv_pass(x)
            x = x + self.center_crop_3d(residual, x.shape[2:])
            return self.last_activation(x)
...
class AttentionBlockModule(nn.Module):
    ...
    def __init__(self, F_g, F_l, F_int, dims, upsample_factor=None, batch_norm=False):
    ...
    def calculate_and_apply_padding(self, smaller_tensor, larger_tensor):
        """
        Calculate and apply symmetric padding to the smaller tensor to match
        the dimensions of the larger tensor.
        """
        padding = []
...
model = UNetLSD(**network_config['args'])
#%%

#%%
# load chckpoint_path
checkpoint = torch.load(chckpoint_path, map_location="cpu", weights_only=True)
...
rand_in = torch.randn(1, 1, *input_shape).float().to(device)
with torch.no_grad():
    out = model(rand_in)
out.shape
#%%
...
classes = ["axons",]* 13
output_channels = 13
output_voxel_size = Coordinate((16, 16, 16))  # z,y,x order
input_voxel_size = Coordinate((16, 16, 16))

read_shape = Coordinate((196, 196, 196))
write_shape = Coordinate((92,92,92))
block_shape = np.array((92,92,92, 13))
# chunk_output_axes = ("z", "y", "x", "c")
